...
class NovelBookNetModel extends BaseModel {
  ...
  NovelBookIntroContentEntity bookIntroContentEntity =
      NovelBookIntroContentEntity();
  ...
  Future<BaseResponse<NovelKeyWordSearch>> searchTargetKeyWord(
  ...
  Future<BaseResponse<NovelShortComment>> getNovelShortReview(String bookId,
      {String sort: 'updated', int start: 0, int limit: 20}) async {
    return _api.getNovelShortReview(bookId,
        sort: sort, start: start, limit: limit);
  }

  Future<BaseResponse<NovelBookReview>> getNovelBookReview(String bookId,
      {String sort: 'updated', int start: 0, int limit: 20}) async {
    return _api.getNovelBookReview(bookId,
        sort: sort, start: start, limit: limit);
  }
  ...
  Future<BaseResponse<NovelBookChapter>> getNovelBookCatalog(
      String sourceId) async {
    return await _api.getNovelCatalog(sourceId);
  }

  Future<BaseResponse<List<NovelBookSource>>> getNovelBookSource(
      String bookId) async {
